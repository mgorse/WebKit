...
static bool isFrameFamiliarWith(AbstractFrame& abstractFrameA, AbstractFrame& abstractFrameB)
{
    ...
    auto *mainFrameA = dynamicDowncast<LocalFrame>(frameA->abstractMainFrame());
    auto *mainFrameB = dynamicDowncast<LocalFrame>(frameB->abstractMainFrame());
...
AbstractFrame* FrameTree::find(const AtomString& name, AbstractFrame& activeFrame) const
{
    ...
    auto* localFrame = dynamicDowncast<LocalFrame>(m_thisFrame);
    for (AbstractFrame* frame = localFrame ? dynamicDowncast<LocalFrame>(localFrame->abstractMainFrame()) : nullptr; frame; frame = frame->tree().traverseNext()) {
        if (frame->tree().uniqueName() == name)
            return frame;
    }
...
AbstractFrame* FrameTree::traverseNext(CanWrap canWrap, DidWrap* didWrap) const
{
    ...
    if (canWrap == CanWrap::Yes) {
        ...
        if (!localFrame)
            return nullptr;
        return dynamicDowncast<LocalFrame>(localFrame->abstractMainFrame());
    }
